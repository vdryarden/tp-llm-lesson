...
To say you *bought* something, add `lon esun` (at the market):

> mi kama jo e len lon esun. -- I bought a shirt.

Do not use `esun` as a transitive verb — `mi esun e len` is non-standard.
`esun` names the marketplace; the purchase is `kama jo … lon esun`.

> mi kama tan ma Losi -- I come from Russia.

To express *arriving at* a place, use `kama lon [place]` (come to be at):

> mi kama lon tomo. -- I arrived home. (I came to be at the house.)

Note: `kama tawa tomo` focuses on the journey toward the house; `kama lon tomo` emphasizes
arriving and being there.

`kama lili` (become small/less) expresses fading or diminishing:
